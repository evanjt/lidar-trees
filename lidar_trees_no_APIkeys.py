...
# Load predefined boundary of the RBG from a shapefile, and get bounding box GeoJSON for Planet API. 
# Ideally, after importing all the las files, a bounding box should emcompass all necessary areas for Planet data. 
# As we know already know all the data is inside the RBG, this is the quickest implementation as I already had
# this as a shapefile
...
RBG_bbox = shapely.geometry.box(RBG_bbox[0], RBG_bbox[1], RBG_bbox[2], RBG_bbox[3])
...
def import_lidar(lidar_filename):
    ...
    df['tree_potential'] = (df['Classification']==1) & (df['HeightAboveGround'] >= 0.5)  
...
# A simple euclidean distance function. In 2D space, because our original tree locations were only 2D
...
def process_the_lidar(filename):
    ...
    lidarnumber = str(trees['RBGno'].loc[trees['LASfile'] == 'merged_0000'+str(lidarextension)].values[0])
    
    if os.path.isfile(os.path.join(os.getcwd(),output_folder,'lidar',lidarnumber+'meta.csv')):
        ...
        lep = local_max(df.loc[df['tree_potential'],['X','Y','Z','HeightAboveGround', 'X_0', 'Y_0', 'Z_0']], radius=3, density_threshold=15)
        treetop_spheres = p3.scatter(lep['Y'], lep['Z'], lep['X'], color='black', size=.5, marker='sphere')
        ...
        for tree in lep.values.tolist():
            tree_distances.append(euclid_dist(tree[0], tree[1], centroid[0], centroid[1]))
        ...
        centre_tree_df = df.loc[df['tree_idx'] == df.loc[closest_treetop.index[0]]['tree_idx']]
        ...
        x1,y1 = closest_treetop[['X_0', 'Y_0']].values[0][0],closest_treetop[['X_0', 'Y_0']].values[0][1]
        x2,y2 = transform(inProj,outProj,x1,y1)

        # Tree's coordinates in WGS84
        tree_coords_wgs84 = (x2,y2)
        tree_coords_wgs84utm = (x1,y1)
        print(lidarnumber, "- WGS84 UTM coordinates are:\t", str(tree_coords_wgs84utm[1])+','+str(tree_coords_wgs84utm[0]))
        print(lidarnumber, "- WGS84 long coordinates are:\t", str(tree_coords_wgs84[1])+','+str(tree_coords_wgs84[0]))

        if not os.path.isdir(os.path.join('.',output_folder)):
            os.makedirs(os.path.join('.',output_folder))
            print (lidarnumber, "- Created folder", os.path.join('.',output_folder))
        if not os.path.isdir(os.path.join('.',output_folder, 'lidar')):
                os.makedirs(os.path.join('.',output_folder, 'lidar'))
                print (lidarnumber, "- Created folder", os.path.join('.',output_folder, 'lidar'))

        # Volume and height of tree cloud (in cubic metres, expecting coordinate system is metres - UTM WGS84 in this case)
        tree_volume = round(scipy.spatial.ConvexHull(centre_tree_df[['X','Y','Z']]).volume,2)
        tree_height = round(closest_treetop['HeightAboveGround'].values[0],2)
        print(lidarnumber, "- Dimensions of the tree are:\t Volume:", str(tree_volume)+"m^3", "Height:", str(tree_height)+"m")

        # Save lidar 3D plot
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.dist = 8
        ax.scatter(centre_tree_df['X'].values, centre_tree_df['Y'].values, centre_tree_df['Z'].values, zdir="z", s=1, marker='^')
        ax.view_init(elev=10., azim=0)
        fig.savefig(os.path.join(os.getcwd(),output_folder,'lidar',lidarnumber+'_plot0deg.jpg'), dpi=400)
        print(lidarnumber, "- Saved 0° plot to:\t\t", os.path.join('.',output_folder,'lidar',lidarnumber+'_plot0deg.jpg'))
        ax.view_init(elev=10., azim=45)
        fig.savefig(os.path.join(os.getcwd(),output_folder,'lidar',lidarnumber+'_plot45deg.jpg'), dpi=400)
        print(lidarnumber, "- Saved 45° plot to:\t\t", os.path.join('.',output_folder,'lidar',lidarnumber+'_plot45deg.jpg'))
        ax.view_init(elev=10., azim=90)
        fig.savefig(os.path.join(os.getcwd(),output_folder,'lidar',lidarnumber+'_plot90deg.jpg'), dpi=400)
        print(lidarnumber, "- Saved 90° plot to:\t\t", os.path.join('.',output_folder,'lidar',lidarnumber+'_plot45deg.jpg'))
        plt.close()

        # Obtain nearmap imagery
        nearmap_url = "http://au0.nearmap.com/staticmap?center="+str(tree_coords_wgs84[1])+","+str(tree_coords_wgs84[0])+"&size=3000x3000&zoom=22&date="+todays_date+"&httpauth=false&apikey="+NEARMAP_API_KEY
        fn, _ = urllib.request.urlretrieve(nearmap_url, os.path.join(os.getcwd(),output_folder,'lidar',lidarnumber+'_aerial.jpg'))
        print(lidarnumber, "- Saved Nearmap imagery to:\t", os.path.join('.',output_folder,'lidar',lidarnumber+'_aerial.jpg'))
        
        import csv
        with open(os.path.join(os.getcwd(),output_folder,'lidar',lidarnumber+'meta.csv'), 'w') as csvfile:
            lidar_metawriter = csv.writer(csvfile, delimiter=',')
            lidar_metawriter.writerow(['treenumber', 'long', 'lat', 'utm_x', 'utm_y', 'height', 'tree_volume'])
            lidar_metawriter.writerow([lidarnumber, tree_coords_wgs84[0], tree_coords_wgs84[1], tree_coords_wgs84utm[0], tree_coords_wgs84utm[1], tree_height, tree_volume])
        print(lidarnumber, "- Written csv file: ./"+output_folder+"/lidar/"+lidarnumber+"meta.csv")

def run_lidar_multithread():
    pool = Pool()
    # Start the larger files first. Will finish quicker this way
    pool.map(process_the_lidar, sorted(glob.iglob(os.getcwd()+'/clipped/*.las'), key=os.path.getsize, reverse=True))
    pool.close()
    pool.join()
...
def concat_feature_list(features, RBG_bbox):
    ...
    for f in features:
        ...
        feature_list.append([date[2], date[1], date[0], f['id'], 
                             f['properties']['cloud_cover'], f['_links']['assets'], f['properties']['sun_azimuth'], f['properties']['sun_elevation'], f['properties']['view_angle'], feature_bbox.contains(RBG_bbox)])
    return feature_list
...
def acquire_planet_data(feature_asset_url, output_folder, session):
    ...
    filename = pl_download(location_url, output_folder)
...
def get_planet_data(output_folder):
    ...
    while len(features) == 250:
        ...
        geojson = res.json()
        features = geojson["features"]
        suitable_images = suitable_images.append(pd.DataFrame(concat_feature_list(features, RBG_bbox), 
                                                              columns=['day', 'month', 'year', 'id', 'cloud', 'asset_url', 'sun_azimuth', 'sun_elevation', 'view_angle', 'fits_project']))
        print("Features:", len(features), "Total features:", len(suitable_images))
...
def crop_the_planet():
    ...
    for tif in glob.iglob(os.path.join(os.getcwd(),'output','ndvi','*.tif')):
        with rasterio.open(tif) as src:
            ndvi = src.read(1)
        png_filename = os.path.join('.','output','ndvi',tif.split('.')[0]+'ndvi_cmap.png')
...
def import_lidar_csvs(output_folder):
    lidar_data = pd.DataFrame(columns=['treenumber', 'long', 'lat', 'utm_x', 'utm_y', 'height', 'tree_volume'])
    for file in glob.iglob(os.path.join(os.getcwd(),output_folder,'lidar','*.csv')):
...
def plot_lidar_data(lidar_data, output_folder):
    ...
    for i in range(len(lidar_data)):
        plot = lidar_data.iloc[i].filter(regex='20').plot(figsize=(20,5), title="Tree: "+str(lidar_data.iloc[i]['treenumber'])+"'s "+plot_title)
        plt.xticks(np.arange(25), lidar_data.iloc[i].index[7:])
        #plot.tick_params(labelbottom=False, direction='out', length=6, width=2, axis='both')
        fig = plot.get_figure()
        fig.savefig(os.path.join(os.getcwd(),'./',output_folder,'lidar',str(lidar_data.iloc[i][0])+"_ndvi_plot.png"))
        plt.close()
        
def output_pdf(lidar_data, pdf_filename):
    # Join the two shapefiles together to get both extracted lidar data, as well as tree information
    tree_data = gpd.read_file(las_shapefile)
    tree_data = tree_data.set_index('RBGno').join(lidar_data.set_index('treenumber'), how='inner')
    
    # Create PDF. A lot of this is manual, especially the begining. Would need editing 
    # for another project
    ...
    c.drawString(175,230, "Nearmap latest imagery as of: "+str(datetime.datetime.today().strftime('%d-%m-%Y')))
    c.drawString(175,210, "Planet data series from: "+str(datetime.datetime.today().strftime('10-2016')))
    c.drawString(175,190, "Planet data series until: "+str(datetime.datetime.today().strftime('10-2018')))
    ...
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201610ndvi_cmap.png'), 20, 650, 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201611ndvi_cmap.png'), 20+(4*cm), 650, 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201612ndvi_cmap.png'), 20+(8*cm), 650, 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201701ndvi_cmap.png'), 20+(12*cm), 650, 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201702ndvi_cmap.png'), 20+(16*cm), 650, 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201703ndvi_cmap.png'), 20, 650-(4*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201704ndvi_cmap.png'), 20+(4*cm), 650-(4*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201705ndvi_cmap.png'), 20+(8*cm), 650-(4*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201706ndvi_cmap.png'), 20+(12*cm), 650-(4*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201707ndvi_cmap.png'), 20+(16*cm), 650-(4*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201708ndvi_cmap.png'), 20, 650-(8*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201709ndvi_cmap.png'), 20+(4*cm), 650-(8*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201710ndvi_cmap.png'), 20+(8*cm), 650-(8*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201711ndvi_cmap.png'), 20+(12*cm), 650-(8*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201712ndvi_cmap.png'), 20+(16*cm), 650-(8*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201801ndvi_cmap.png'), 20, 650-(12*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201802ndvi_cmap.png'), 20+(4*cm), 650-(12*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201803ndvi_cmap.png'), 20+(8*cm), 650-(12*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201804ndvi_cmap.png'), 20+(12*cm), 650-(12*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201805ndvi_cmap.png'), 20+(16*cm), 650-(12*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201806ndvi_cmap.png'), 20, 650-(16*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201807ndvi_cmap.png'), 20+(4*cm), 650-(16*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201808ndvi_cmap.png'), 20+(8*cm), 650-(16*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201809ndvi_cmap.png'), 20+(12*cm), 650-(16*cm), 3.8*cm, 3.8*cm)
    c.drawImage(os.path.join(os.getcwd(), 'output','ndvi', '201810ndvi_cmap.png'), 20+(16*cm), 650-(16*cm), 3.8*cm, 3.8*cm)

    c.drawString(140,150, "Planet (PlanetScope4) data" )
    c.drawString(140,130, "Applying Normalised Difference Vegetation Index (NDVI)")
    ...
    for tree in tree_data.iterrows():
        ...
        # Align images
        c.drawImage('./output/lidar/'+str(tree_id)+'_aerial.jpg', 200, 450, 10*cm, 10*cm)
        c.drawImage('./output/lidar/'+str(tree_id)+'_plot0deg.jpg', -100, 540, 10*cm, 10*cm)
        c.drawImage('./output/lidar/'+str(tree_id)+'_plot45deg.jpg', -100, 300, 10*cm, 10*cm)
        c.drawImage('./output/lidar/'+str(tree_id)+'_plot90deg.jpg', -100, 60, 10*cm, 10*cm)
        c.drawImage('./output/lidar/'+str(tree_id)+'_ndvi_plot.png', 50, -50, 17.4*cm, 5*cm)
        # Text for trees
        c.setFillColor('black')
...
def main():
    ...
    satellite_data = get_planet_data(output_folder)
    crop_the_planet()

    # Let's multitask this to use all the processes available. This works fine with this dataset with 12 threads
    # and 32gb RAM. But my overload the ram as there seems to be a problem clearing the ram after each 
    # thread has been run.
    print("\n------- Scanning LiDAR Data -------")
    run_lidar_multithread()

    # Import the CSV data for each LiDAR, created during the previous step. If it had been processed already, it is 
    ...
    for tif in sorted(glob.iglob(os.path.join(os.getcwd(),'output','ndvi','*.tif'))):
        ndvi_date = tif.split('/')[-1].split('.')[0]
        lidar_data[ndvi_date] = lidar_data.apply(lambda row: ndvi_at_coordinate('./output/ndvi/'+ndvi_date+'.ndvi.tif',row['utm_x'],row['utm_y']), axis=1)
